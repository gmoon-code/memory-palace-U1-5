...
later_lock_path=ROOT/'content/ap-biology/unit-5/content-lock-f6.json'
later_runtime=read('../unit-5/content-lock-f6.json').get('runtime_file_sha256',{}) if later_lock_path.exists() else {}
for rel,digest in lock.get('runtime_file_sha256',{}).items():
    p=ROOT/rel
    current=sha(p) if p.exists() else None
    if current!=digest and later_runtime.get(rel)!=current: problems.append(f'F6 runtime hash mismatch without later validated replacement: {rel}')
...
if health.get('version') not in {'v2-apbio-0.22.0-u4-f6','v2-apbio-0.23.0-u5-f5','v2-apbio-0.24.0-u5-f6'}: problems.append(f"Unexpected API version after Unit 4 F6: {health.get('version')}")
