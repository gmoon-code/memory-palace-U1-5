...
for rel,digest in lock.get('runtime_file_sha256',{}).items():
    p=ROOT/rel
    if not p.exists() or sha(p)!=digest: problems.append(f'F6 runtime hash mismatch: {rel}')
...
if health.get('version')!='v2-apbio-0.24.0-u5-f6': problems.append(f"Unexpected API version after Unit 5 F6: {health.get('version')}")
