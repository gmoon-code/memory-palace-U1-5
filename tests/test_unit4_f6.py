...
def test_unit4_f6_content_and_runtime_lock():
    ...
    later=ROOT/'content/ap-biology/unit-5/content-lock-f6.json'
    ...
    for rel,digest in lock['runtime_file_sha256'].items():
        ...
        assert current==digest or later_runtime.get(rel)==current

def test_unit4_f6_live_api():
    assert client.get('/api/health').json()['version'] in {'v2-apbio-0.22.0-u4-f6','v2-apbio-0.23.0-u5-f5','v2-apbio-0.24.0-u5-f6'}
